...
RFM 使用最近购买间隔、去重购买会话数和有效购买金额进行评分。由于 78.91% 的购买用户仅有一个购买会话，F 未机械使用 `NTILE(5)`，而是根据会话次数设置固定阈值；R 和 M 使用 `PERCENT_RANK()`，避免相同取值被强行拆分。若购买用户的全部购买事件均缺失 `user_session`，基础表会保留该用户用于数据质量审计，但不会将 F=0 误判为最高频用户，也不会纳入 RFM 评分和分群。

MySQL 验证结果显示 `zero_session_purchase_users = 0`，因此边界修复没有改变现有结果。110,518 名购买用户被划分为 8 类。四类重要用户合计占 39.99%，贡献 78.28% 的有效交易金额估算；其中 10.33% 的重要价值用户贡献 31.40%。重要保持与重要挽留用户合计贡献 34.98%，但平均最近购买间隔分别达到 89.03 天和 103.34 天，是优先召回对象。
