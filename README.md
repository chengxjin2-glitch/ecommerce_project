...
Power BI 看板
查看完整三页 PDF 报告 · 下载 Power BI 源文件 · 查看 Power BI 设计与 DAX 说明
经营概览
...
原始数据粒度：一行代表一次用户行为事件
原始 CSV 体积较大，不纳入本仓库。下载和导入方式见 data/README.md。
项目结构
```text
ecommerce_project/
├── README.md
...
```
执行顺序
将五个月度 CSV 导入 MySQL，表名分别设置为 `2019_oct`、`2019_nov`、`2019_dec`、`2020_jan`、`2020_feb`。
按编号依次运行 `sql/` 下的脚本。
所有业务分析均使用清洗表 `ecommerce_clean`，原始汇总表 `ecommerce_all` 只用于审计和追溯。
原始表和清洗表脚本按“一次性执行”设计；07–09 会重建对应的派生结果表，重新运行前请确认这些表未被其他流程依赖。
数据审计与清洗
