...
基于约 2,069 万条美妆电商用户行为日志完成数据审计、清洗、核心指标体系、趋势分析、三层转化漏斗、品类经营下钻和 RFM 用户价值分层。项目使用 MySQL 8.0，重点展示从原始行为日志到可复用分析表的完整 SQL 链路。

> 项目状态：SQL 分析主体已完成；Power BI 仪表盘和性能对比实验待补充。
